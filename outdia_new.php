        ...
        <?php
         ...
         if ($result) {
            ...
            while($row = mysqli_fetch_array($result)) {
               ...
               // Si cambia de valor se pinta
               if ($row["HORA"] > $vhora) {
                    $vresta = $row["VALOR"] - $vresta;
                    if ($vresta < 0) { $vresta =0;}
                    array_push($arrData["data"], array(
                       "label" => $vlabel,
                       "value" => posdecimal($vresta,$row["POSDECIMAL"]) // <-- Se hace la resta con el valor anterior.
                       )
                    );
               }
               $vhora =  $row["HORA"];
            ...
            // Pintar última línea se almacena siempre en $fila1
            $vresta = $row["VALOR"] - $vresta;
            if ($vresta < 0) { $vresta =0;}
            array_push($arrData["data"], array(
                "label" => $vlabel,
                "value" => posdecimal($vresta,$fila1["POSDECIMAL"]) // <-- Se hace la resta con el valor anterior.
                )
            );
